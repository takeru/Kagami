...
- **start_playwright_mcp.py**: MCPサーバー起動スクリプト（自動生成）
...
- ✅ Firefox build v1496のインストール（HOME=/home/user）
- ✅ Firefoxプロファイルの作成
...
### MCPサーバー起動スクリプト (start_playwright_mcp.py)

**実行:**
```bash
HOME=/home/user python playwright_mcp_claude_code_web/start_playwright_mcp.py
```

**動作:**
- proxy.pyを起動（バックグラウンド）
- playwright-mcpを起動（フォアグラウンド）
- Ctrl+Cで両方を停止

**用途:**
- MCPサーバーを単独で起動したい場合
- 他のMCPクライアントから接続したい場合
...
- [ ] `setup.sh`を`HOME=/home/user`で実行した
...
- [ ] CA証明書がインポートされている（`certutil -L`で確認）
- [ ] `example.py`が成功する
