"""
Firefoxを使ってCloudflare Turnstileを突破（修正版）

mainブランチのサンプルから学んだ方法：
- launch_persistent_context()ではなくlaunch()を使用
- env={"HOME": temp_home}で一時HOMEディレクトリを設定
"""
...
def test_firefox_cloudflare():
    """FirefoxでCloudflare Turnstile突破を試みる（修正版）"""
    proxy_process = None

    try:
        print("=" * 70)
        print("Firefox（修正版）でCloudflare Turnstile突破")
        print("=" * 70)
        ...
        print("\n" + "=" * 70)
        print("Test: Firefox + launch() + 一時HOME")
        print("=" * 70)

        with sync_playwright() as p:
            print("\n[1] Firefoxを起動（mainサンプルの方法）...")

            # 一時的なHOMEディレクトリを作成（mainサンプルから）
            temp_home = tempfile.mkdtemp(prefix="firefox_home_", dir="/tmp")

            browser = p.firefox.launch(  # launch_persistent_context()ではなくlaunch()
                headless=True,
                proxy={
                ...
                env={
                    **os.environ,
                    "HOME": temp_home,  # 一時HOMEディレクトリ（重要！）
                }
            )
            print("   ✅ Firefox起動完了（mainサンプル方式）")
            ...
            print("\n[8] スクリーンショット...")
            page.screenshot(path="/home/user/Kagami/firefox_fixed_test.png")
            print("    ✓ スクリーンショット保存: firefox_fixed_test.png")

            print("\n[9] HTMLを保存...")
            with open("/home/user/Kagami/firefox_fixed_test.html", "w", encoding="utf-8") as f:
                f.write(content)
            print("    ✓ HTML保存: firefox_fixed_test.html")
        ...
        if success:
            ...
            print("  ✓ launch() + new_page()（mainサンプル方式）")
            print("  ✓ 一時HOMEディレクトリ設定")
