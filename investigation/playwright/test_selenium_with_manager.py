#!/usr/bin/env python3
"""
Selenium + webdriver-managerで基本的なテスト
- webdriver-managerで正しいChromeDriverを自動取得
- プロキシなしで動作するか確認
"""
import sys
...
print("="*60, flush=True)
print("Selenium + webdriver-manager Test", flush=True)
print("="*60, flush=True)
print(flush=True)
...
try:
    ...
    print("Getting ChromeDriver with webdriver-manager...", flush=True)
    sys.stdout.flush()

    # webdriver-managerでChromeDriverを自動取得
    service = Service(ChromeDriverManager(chrome_type=ChromeType.CHROMIUM).install())

    print("✅ ChromeDriver installed", flush=True)
    print("\nLaunching Chrome with Selenium...", flush=True)
    sys.stdout.flush()
